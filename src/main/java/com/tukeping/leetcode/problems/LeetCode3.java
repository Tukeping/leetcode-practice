...
public class LeetCode3 {
    ...
    public int lengthOfLongestSubstring3(String s) {
        ...
        for (int i = 0, j = 0; j < s.length(); j++) {
            i = Math.max(i, slidingWindow[s.charAt(j)]);
            maxLength = Math.max(maxLength, j - i + 1);
            slidingWindow[s.charAt(j)] = j + 1;
        }
        return maxLength;
    }

    /** sliding window optimized 1, time: O(n) space: O(min(m,n)) **/
    public int slidingWindowOptimized(String s) {
        HashMap<Character, Integer> slidingWindow = new HashMap<>();
        int maxLength = 0;
        for (int i = 0, j = 0; j < s.length(); j++) {
            if (slidingWindow.containsKey(s.charAt(j))) {
                i = Math.max(slidingWindow.get(s.charAt(j)), i);
            }
            maxLength = Math.max(maxLength, j - i + 1);
            slidingWindow.put(s.charAt(j), j + 1);
        }
        return maxLength;
    }
    ...
    public int slidingWindow(String s) {
        ...
        int i = 0, j = 0, maxLength = 0, len = s.length();
        while (i < len && j < len) {
    ...
    @Test
    public void test5() {
        int n = lengthOfLongestSubstring("bbtablud");
        assertThat(n, is(6));
    }
}
